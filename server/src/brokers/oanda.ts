...
export class OandaBroker extends StubBrokerAdapter {
  readonly id = 'oanda'; readonly name = 'OANDA';
  constructor() { super(['apiKey', 'accountId']); }
  async connect(creds: BrokerCredentials): Promise<AccountInfo> {
    this.requireCredentials(creds);
    const host = creds.server === 'OANDA-Live' ? 'api-fxtrade.oanda.com' : 'api-fxpractice.oanda.com';
    const response = await fetch(`https://${host}/v3/accounts/${creds.accountId}`, { headers: { Authorization: `Bearer ${creds.apiKey!}` } });
    ...
    return accountFrom(creds, creds.server || 'OANDA-Practice', { ...account, balance: account.balance, equity: account.NAV, margin: account.marginUsed, freeMargin: account.marginAvailable, leverage: 100 });
  }
}
